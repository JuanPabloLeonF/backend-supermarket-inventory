package dev.juanleon.supermarket_inventory.modules.cash_register.application.dto;
...
public class CashRegisterRequest {
    ...
    @Digits(integer = 12, fraction = 2, message = "finalAmount has invalid format (max 12 integers and 2 decimals)")
    private BigDecimal finalAmount;

    @NotNull(message = "totalSales is required")
    @PositiveOrZero(message = "totalSales must be zero or positive")
    @Digits(integer = 12, fraction = 2, message = "totalSales has invalid format (max 12 integers and 2 decimals)")
    private BigDecimal totalSales;

    @NotNull(message = "totalSales is required")
    @PositiveOrZero(message = "totalPurchases must be zero or positive")
    @Digits(integer = 12, fraction = 2, message = "totalPurchases has invalid format (max 12 integers and 2 decimals)")
    private BigDecimal totalPurchases;

    @NotNull(message = "difference is required")
    @Digits(integer = 12, fraction = 2, message = "difference has invalid format (max 12 integers and 2 decimals)")
