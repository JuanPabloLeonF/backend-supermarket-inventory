...
public class GetAllByIdProviderPurchaseQueryHandler implements IRequestHandler<GetAllByIdProviderPurchaseQuery, PagedResponse<ResponsePurchaseDto>> {
    ...
    @Override
    public PagedResponse<ResponsePurchaseDto> handle(GetAllByIdProviderPurchaseQuery request) {
        PaginationRequest data = PaginationRequest.builder()
                .page(request.page())
                .size(request.size())
                .build();
        PagedResponse<PurchaseModel> pagedResponse = this.iGetPurchaseService.getAllByIdProvider(
                request.idProvider(),
                data
        );
    ...
    @Override
    public Class<GetAllByIdProviderPurchaseQuery> getRequestType() {
        return GetAllByIdProviderPurchaseQuery.class;
    }
}
