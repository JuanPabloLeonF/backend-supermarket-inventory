...
public class GetByPeriodReportQueryHandler implements IRequestHandler<GetByPeriodReportQuery, PagedResponse<ResponseReport>> {
    ...
    public PagedResponse<ResponseReport> handle(GetByPeriodReportQuery request) {
        PaginationRequest data = PaginationRequest.builder()
                ...
                .size(request.size())
                .build();
        PagedResponse<ReportModel> reportModelPagedResponse = this.iGetReportService.getByPeriod(request.period(), data);
        return this.iMapperPaginationApp.pageResponseToPageResponseTypeResponse(
                reportModelPagedResponse,
                this.iMapperReportApplication::toResponse
        );
    }
